...
// Styles
import * as S from './PhotoOrDocument.styles';
...
const PhotoOrDocument = ({ onClose, onSelectDocument }: UploadFileModalProps) => {
  ...
  return (
    <S.Modal>
      <S.Wrapper>
        <S.Hero>
          <h2>Adicionar foto ou documento</h2>
        ...
        <S.Component onDragOver={onDragOver} onDrop={onDrop}>
          {selectedImage ? (
            ...
            <S.Container>
              <S.Header>
                <S.Label>Foto ou Documento</S.Label>
        ...
        <S.Actions>
          <Button.Action
            label={selectedImage ? 'Remover Documento' : 'Sair'}
            onClick={() => {
              if (selectedImage) {
                setSelectedImage('');
              } else {
...
export default PhotoOrDocument;
